...
class Paginator {
    ...
    public static function paginatePosts(Collection $posts, $offset, $num, $conv = false)
    {
//      return $posts->slice($offset, $num)->each(function($post) {
//          $post->convert();
//      });

        return $posts->slice($offset, $num)->map(function($post) use ($conv) {
            return $post->payload($conv);
        });
    }
